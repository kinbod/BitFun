...
- boundary check 覆盖 tool truncation recovery presentation、confirmation wait-result、light checkpoint summary、batching plan、retry policy、DeepReview task-execution 纯策略、provider capacity error decision、provider/admission queue step decision 和 DeepResearch citation renumber 纯重排 owner，要求 core tool pipeline / tool context / DeepReview task adapter / DeepResearch citation hook 委托 `bitfun-agent-tools`、`bitfun-agent-runtime` 和 `tool-runtime::pipeline`。
