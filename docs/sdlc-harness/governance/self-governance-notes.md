...
> 边界：本文只用于内部验证和自我改进；目标项目默认流程、技术栈和质量要求以主设计及项目配置为准。

## 1. 为什么需要独立文档

BitFun 仓库可以作为可配置工程能力的内部验证项目。主设计文档中的“目标项目”代表任意被用户加载的软件工程；BitFun 仓库的技术栈、模块边界、AGENTS 文档、CI 规则和 fork PR 工作流仅作为高复杂度样本。
...
| 默认流程偏重 | 早期文档把 PR 门禁/证据包作为首个闭环 | 临时工具、演示、个人项目体验过重 | P0 改为快速路径 + 安全边界 |
...
- 风险分类器示例可以覆盖 Rust 工作区、React frontend、Tauri desktop、remote、AI 适配器、深度审查等高风险区域。
