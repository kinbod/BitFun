...
| 沙箱边界可见 | PRD-19 | 执行位置和沙箱等级分层 | 安全边界、质量数据面、证据包 | P0、P1、P3 | 本地 shell、远程 SSH、ACP、MCP、MiniApp、插件适配主机、浏览器/桌面和云端任务用例；验证执行位置、沙箱等级组合、降级原因、授权范围和替代路径 |
| 阶段性用户收益 | PRD-20 | 阶段收益编排、质量一致性检查 | 实施计划、指标规格、质量数据面 | P-1、P0-P4 | 每阶段回放低风险任务、安全确认、远程/不可支持状态、PR/团队场景；验证用户收益、技术前置、延期边界和质量一致性 |
