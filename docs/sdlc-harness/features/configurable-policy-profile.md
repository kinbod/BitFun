...
> 模块角色：汇总任务意图、操作风险、执行环境、项目规则、团队策略和用户覆盖，生成当前任务的治理模式、提示形态、验证要求、证据展示层级和安全动作。

## 1. 模块定位

配置化策略画像是 BitFun 的体验调度和策略合成层。它把分散的项目规则、路径规则、团队策略、用户偏好和当前任务信号合成为一个可解释的运行态决策：

```text
当前任务使用哪种治理模式，展示哪些提示，运行哪些检查，哪些动作需要安全确认？
```

项目级配置提供默认值；任务意图、目标路径、工具能力、网络/凭据/发布链路、团队规则和用户覆盖共同决定最终运行态。

## 2. 产品原则

- **配置优先**：优先读取项目、路径、团队和用户配置，并保留来源、作用域和新鲜度。
- **任务定态**：治理模式由当前任务和动作决定，同一项目内的文档变更、热修复、迁移和演示原型可以使用不同模式。
- **快速默认**：缺少高风险信号时使用 `fast` 或 `assist`，只展示结果摘要、推荐检查和未验证项。
...
| 环境信任度 | 临时目录、git worktree、团队仓库、远程沙箱、连接生产环境的工作区 | 决定可自动执行范围 |
...
| 治理模式 | 默认体验 | 典型场景 |
|---|---|---|
| `fast` | 低摩擦执行、简短摘要、低成本验证建议，不生成完整证据包 | 临时脚本、演示、文档、小工具、探索性改动 |
| `assist` | 提示推荐检查、展示未验证项，但不阻塞 | 常规本地开发、小范围界面(UI)/适配器改动 |
...
治理模式是当前任务的运行态。项目可以声明默认模式，用户可以在当前任务或当前会话临时覆盖；覆盖范围、期限和残余风险进入质量数据面记录。
...
用户画像用于选择默认入口、解释语言和提示密度。强制检查、阻断和安全确认继续引用任务风险、执行动作、团队策略或安全边界。
...
| 场景 | 策略 |
|---|---|
| 无 git 临时目录 | 默认 `fast`；跨目录写和网络仍走安全边界 |
| 用户要求“赶紧写个小工具” | 保持 `fast`；只在 shell/网络/凭据/删除时打断 |
